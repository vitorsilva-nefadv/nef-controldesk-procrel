...
from .sudoeste_processadorv2 import processar_sudoeste_processadorv2
...
@app.post("/sudoeste-processadorv2")
async def sudoeste_processadorv2(
    request: Request,
    consolidada: UploadFile | None = File(None),
    direta: UploadFile | None = File(None),
    indireto: UploadFile | None = File(None),
):
    multipart_keys = list((await request.form()).keys())
    return await _executar_fluxo_upload(
        ...
        uploads={"consolidada": consolidada, "direta": direta, "indireto": indireto},
        processor=processar_sudoeste_processadorv2,
        output_filename="sudoeste_processadorv2_resultado.xlsx",
        multipart_keys=multipart_keys,
        processor_param_map={
            "consolidada_excel": "consolidada",
            "direta_excel": "direta",
            "indireto_excel": "indireto",
        },
    )
